...
class ExperienceForwardTransfer(GenericExperienceForwardTransfer):
    ...
    def __str__(self):
        return f"ExperienceForwardTransfer_{self.from_state_key}"
...
class StreamForwardTransfer(GenericStreamForwardTransfer):
    ...
    def __str__(self):
        return f"StreamForwardTransfer_{self.from_state_key}"


def forward_transfer_metrics_from_strategy_state(*,  state_keys: List[str], experience=False, stream=False) -> List[PluginMetric]:
    """
    ...
    for state_key in state_keys:
        if experience:
            metrics.append(ExperienceForwardTransfer(from_state_key=state_key))

        if stream:
            metrics.append(StreamForwardTransfer(from_state_key=state_key))
