...
from continual_learning.models.continual.ensemble.ensemble_e2e_module_reproduction import \
    EnsembleE2EModule
from continual_learning.models.continual.ensemble.ensemble_e2e_pytorch_reproduction import \
    EnsembleE2E, ensemble_loss, create_default_dirname
...
def setup_and_run(config: DictConfig, logging_dir: Path) -> None:
    ...
    for index, train_task in enumerate(train_stream):
...
if __name__ == '__main__':
    common_config = {
        'ensemble_learning_rate': 0.0001, 'ensemble_weight_decay': 0.0001, 'tanh_factor': 250.0, 'trainable_keys': False,
        'device': 'cuda', 'train_epochs': 1, 'return_task_id': False, 'shuffle': True, 'augment_data': False,
        'num_generated_batches': 1, 'num_generated_examples': 20, 'fixed_class_order': None, 'setup': 'Reproduction'
    }
